...
Crackle has two outputs: audio and digital.  Audio output generates pops at different volumes, but suffers from GRAINS's low-level PWM whine.  Digital generates clean pops, but they are all at maximum volume and the default value is 0V with the pop at 5V, which means it's not centered for audio and perhaps less useful as a result -- maybe more useful for CV?
