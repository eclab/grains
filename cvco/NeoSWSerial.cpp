...
static volatile uint8_t *txPort2;  // port register
...
void NeoSWSerial::listen()
{
  ...
  if (txPin2 != 255)
  	{
	  txBitMask = digitalPinToBitMask( txPin2 );
	  txPort2    = portOutputRegister( digitalPinToPort( txPin2 ) );
	  if (txPort2)
		*txPort2  |= txBitMask;   // high = idle
