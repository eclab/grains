...
// The lowest frequency, corresponding with 0V, is 32.7.  Frequencies go up exponentially
// such that at frequencies[1023] we are 5V (5 octaves) higher than frequencies[0].  We then
// continue another 512 for some headroom when pitch-shifting.
...
// These make it more convenient to define chords by note (relative to C), rather than by pitch number
...
#define CHORDS(c,n) pgm_read_byte_near(&chords[c][n])
...
const uint8_t releases[5] = { 16, 32, 64, 128, 255 };           // these are the possible values for counters
...
void trigger()
    ...
    counters[nextSine] = max[nextSine] = releases[release];
    nextSine++;
